...
describe Cursed::Interpreter do
  it "transpiles-then-evals an expression" do
    expect(Cursed::Interpreter.new.eval_str("(+ 1 2)")).to eq 3
  end

  it "evaluates core seq mappings against real Ruby" do
    expect(Cursed::Interpreter.new.eval_str("(reduce + (map inc [1 2 3]))")).to eq 9
  end
  ...
  it "exposes the emitted Ruby for a form" do
    form = Cursed::Reader.read_all("(map inc xs)").first
